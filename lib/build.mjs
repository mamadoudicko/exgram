 ...
 * @property {string} [icon]                    generic category icon (e.g. 'database', 'queue') or a data: URL; embedded as an image. Unresolvable values fall back to the role color. (#24)
 */
/**
...
export function buildScene(spec = {}) {
  ...
  const arrowGeo = [];
  const files = {};       // fileId -> embedded image bytes (icons, #24)
  const iconWarnings = []; // node icons that couldn't be resolved offline
  ...
  for (const n of nodes) {
    ...
    // #24: optional icon. Resolve OFFLINE; on success embed it as an image
    // element pinned to the top-left inset and nudge the label right so they
    // don't overlap. On failure render exactly as before + warn the author.
    let labelX = n._x + 8, labelW = BOX_W - 16;
    if (n.icon != null && n.icon !== '') {
      const resolved = resolveIcon(n.icon);
      if (resolved) {
        const ICON = 28, INSET = 12;
        // fileId derived from the icon value so rebuilds reuse the same bytes
        // and Excalidraw can reconcile the image element/file by id.
        const fileId = 'icon-' + hash31('icon:' + String(n.icon)).toString(16);
        if (!files[fileId]) {
          const dataURL = /^data:/i.test(resolved.svg)
            ? resolved.svg
            : `data:${resolved.mimeType};base64,` + Buffer.from(resolved.svg, 'utf8').toString('base64');
          files[fileId] = {
            id: fileId,
            mimeType: resolved.mimeType,
            dataURL,
            created: stamp.updated,
            lastRetrieved: stamp.updated,
          };
        }
        elements.push(baseElement(stamp, {
          id: `n.${n.id}.icon`, type: 'image',
          x: n._x + INSET, y: n._y + boxH / 2 - ICON / 2, width: ICON, height: ICON,
          strokeColor: 'transparent', backgroundColor: 'transparent',
          extra: { fileId, status: 'saved', scale: [1, 1] },
        }));
        // shift the bound label clear of the icon column
        labelX = n._x + INSET + ICON + 6;
        labelW = BOX_W - (INSET + ICON + 6) - 8;
      } else {
        iconWarnings.push(`node "${n.id}": icon "${n.icon}" not resolvable (only generic category icons + data: URLs are bundled today)`);
      }
    }
    elements.push(textElement(stamp, {
      id: txtId, x: labelX, y: n._y + boxH / 2 - textH / 2, width: labelW, height: textH,
  ...
  // #19: raw elements were spread over only bookkeeping fields and never got an
  // `id` or the render defaults (fillStyle/strokeWidth/roughness/points/...).
  // Excalidraw's updateScene doesn't backfill those, so an id-less element crashed
  // hit-testing and blanked the whole canvas. Normalize each raw element through
  // the SAME defaults baseElement()/textElement() give nodes/edges, with `...raw`
  // taking precedence so authored values always win.
  if (Array.isArray(spec.rawElements)) {
    spec.rawElements.forEach((raw, i) => {
      // Stable id: author's id if any, else derived from the content + index so a
      // rebuild reproduces it (matching the deterministic ids elsewhere).
      const id = raw.id || `raw.${hash31(JSON.stringify(raw) + i)}`;
      const base = baseElement(stamp, {
        id,
        type: raw.type,
        x: raw.x, y: raw.y, width: raw.width, height: raw.height,
        strokeColor: raw.strokeColor,
        backgroundColor: raw.backgroundColor,
        fillStyle: raw.fillStyle,
        strokeWidth: raw.strokeWidth,
        strokeStyle: raw.strokeStyle,
        roughness: raw.roughness,
        roundness: raw.roundness ?? null,
        seed: raw.seed, // baseElement derives hash31(id) when absent
      });
      // Type-specific render defaults so Excalidraw never reads an undefined field.
      let typeDefaults = {};
      if (raw.type === 'text') {
        ...
        typeDefaults = {
          // default to a straight segment spanning the element's box
          points: [[0, 0], [raw.width ?? 0, raw.height ?? 0]],
          lastCommittedPoint: null,
          startBinding: null,
          endBinding: null,
          startArrowhead: null,
          endArrowhead: raw.type === 'arrow' ? 'arrow' : null,
          elbowed: false,
        };
      }
      // `...raw` last: authored values win over every default above.
      elements.push({ ...base, ...typeDefaults, ...raw, id });
    });
  }
  ...
  Object.defineProperty(scene, '_warnings', { value: [...iconWarnings, ...lintLayout(boxes, arrowGeo)], enumerable: false });
  return scene;
}
