...
// The HTTP server instance, hoisted to module scope so the /api/shutdown handler
// can close the listener (used by the version-aware handoff in start()).
let server = null;
...
const handler = async (req, res) => {
  ...
  // Create a brand-new BLANK board: writes an empty Excalidraw scene to
  // <slug>.excalidraw and, deliberately, NO <slug>.json spec — it's a hand-made
  // board the user fills in on the canvas. The viewer's "＋ New board" button
  // posts here, then opens /?d=<slug>&edit=1. Loopback-only + slug guard cover
  // traversal; the `exists` check refuses to clobber an existing board.
  if (req.method === 'POST' && url === '/api/create') {
    ...
    // Refuse to clobber an existing board under this name (either file).
    if (existsSync(join(WORKSPACE, `${slug}.excalidraw`)) || existsSync(join(WORKSPACE, `${slug}.json`))) {
      return send(res, 409, 'board already exists');
    }
    // A truly blank scene the viewer can load and immediately edit. Shape matches
    // what the viewer serializes back on save (type/version/elements/appState/files).
    const blank = JSON.stringify({ type: 'excalidraw', version: 2, elements: [], appState: {}, files: {} });
    try {
      writeFileSync(join(WORKSPACE, `${slug}.excalidraw`), blank);
    } catch (err) {
      return send(res, 500, `create failed: ${err.message}`);
    }
  ...
  // Graceful shutdown, loopback-only by virtue of the server binding 127.0.0.1.
  // A newer-version server taking over this workspace's port POSTs here so we
  // release the port cleanly (see the handoff in start()); we only ever shut
  // ourselves down. POST-only so a stray GET can't kill the board.
  if (req.method === 'POST' && url === '/api/shutdown') {
    // Server-to-server only: the handoff in start() calls this from a sibling
    // node process, which sends NO Origin. A browser fetch always carries an
    // Origin on a POST, so reject any request that has one. Without this, a
    // no-body POST is a CORS "simple request" (no preflight), so any site the
    // user visits could POST to 127.0.0.1 and kill their board.
    if (req.headers.origin) return send(res, 403, 'forbidden');
    send(res, 200, 'shutting down');
    // Close the listener (frees the port once in-flight requests drain), then
    // exit. A short timer force-exits if a lingering connection holds us open.
  ...
  if (url === '/whoami') {
    return send(
      res,
      200,
      // `pid` lets a newer-version server taking over this workspace SIGTERM us
      // if the graceful /api/shutdown handoff doesn't land (see start()).
      JSON.stringify({ workspace: absWorkspace, version: VERSION, port: boundPort, pid: process.pid }),
      'application/json',
    );
  }
...
// Evict a stale same-workspace server (a DIFFERENT version) holding `port`, so a
// newer build can take it over. We ask it to release the port two ways, then
// wait for it to actually go: (1) POST /api/shutdown for a graceful close;
// (2) SIGTERM its pid (from /whoami) as a fallback for older builds whose
// shutdown endpoint we can't rely on. Resolves true once the port looks free.
// NOTE: a server old enough to lack BOTH a shutdown route and a pid in /whoami
// (pre-handoff builds) can't be evicted remotely and needs one manual restart.
const evictStaleServer = async (port, who) => {
  // (1) graceful shutdown — best effort, ignore status/errors. `agent: false`
  // (no keep-alive) so our socket closes the instant the response lands; a
  // lingering keep-alive connection would block the old server's close() from
  // releasing the port, and the rebind below would then fail.
  await new Promise((resolve) => {
    const req = httpRequest(
      { host: '127.0.0.1', port, path: '/api/shutdown', method: 'POST', timeout: 500, agent: false },
      (res) => {
        res.resume();
        res.on('end', resolve);
      },
  ...
  // (2) fallback: SIGTERM the pid it advertised. Guarded so a reused/foreign pid
  // or an already-exited process can't throw us off (ESRCH/EPERM are ignored).
  // `who.pid` comes from a remote /whoami, so guard the value: pid 0 and
  // negatives target a whole process GROUP and don't throw, so the catch
  // wouldn't save us. Require a real, positive pid.
  if (who && Number.isInteger(who.pid) && who.pid > 0) {
    try {
      process.kill(who.pid, 'SIGTERM');
    } catch {
      /* already gone, or not ours to kill — the wait below decides the outcome */
    }
  }
  // Wait for the port to free: /whoami stops answering once the old process is
  // down. Up to ~3s, which is ample for a local close()+exit.
  for (let i = 0; i < 30; i++) {
    if (!(await probeWhoami(port))) return true;
...
const start = async () => {
  ...
  // Claim a freshly-bound port: record it and publish the URL for this workspace.
  const claim = (port) => {
    boundPort = port;
  ...
  for (let port = PREFERRED_PORT; port <= PREFERRED_PORT + MAX_PORT_PROBE; port++) {
    ...
    // Port in use: is it an exgram serving OUR workspace?
    const who = await probeWhoami(port);
    if (who && who.workspace === absWorkspace) {
      if (who.version === VERSION) {
        // Same workspace AND same version — a genuine equivalent. Reuse it; it
        // already wrote the correct .exgram-url, so don't overwrite it.
        console.error(`exgram: viewer already serving this workspace on http://localhost:${port}`);
        process.exit(0);
      }
      // Same workspace, DIFFERENT version — a stale server. Without this check it
      // would be mistaken for an equivalent and WE'D exit, leaving the old build
      // (and its old routes) in charge across turns (issue #54). Evict it and
      // take the port so the skew self-heals on this respawn.
      console.error(
        `exgram: replacing stale server (v${who.version || '?'}) on port ${port} with v${VERSION}`,
      );
      const freed = await evictStaleServer(port, who);
      if (freed) {
        // Re-bind the now-free port. A just-released port can briefly linger, so
        // retry a few times before giving up on it.
        let rebind = 'inuse';
        for (let attempt = 0; attempt < 5 && rebind !== true; attempt++) {
          if (attempt) await new Promise((r) => setTimeout(r, 100));
          try {
            rebind = await tryBind(server, port);
          } catch (err) {
            console.error(`exgram: bind error after takeover on ${port}: ${err.message}`);
            break;
          }
        }
        if (rebind === true) {
      ...
      // Couldn't evict/rebind — fall through to the next port rather than hang.
      console.error(`exgram: could not take over port ${port}, trying ${port + 1}`);
      continue;
    }
