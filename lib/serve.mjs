...
// $EXGRAM_PORT wins; $PORT is honored too so `portless exgram node lib/serve.mjs`
// (portless assigns the port via $PORT) works with no extra flags.
const PREFERRED_PORT = Number(process.env.EXGRAM_PORT) || Number(process.env.PORT) || 3737;
// Pretty hostname exposed by an optional local reverse proxy (e.g. portless.sh).
// Purely opt-in: if it's reachable AND fronts this workspace, we advertise it;
// otherwise everything falls back to the loopback port. No proxy = no change.
const PRETTY_HOST = process.env.EXGRAM_HOST || 'exgram.localhost';
...
const handler = async (req, res) => {
  ...
  // SPA deep links: a bare /<board-name> serves the viewer, which reads the board
  // name from the path (so `exgram.localhost/checkout-flow` works, same as `?d=`).
  // Must stay AFTER every API route above so it only catches board-name paths.
  if (req.method === 'GET' && SLUG.test(url.slice(1))) {
    return existsSync(VIEWER) ? stream(res, VIEWER, 'text/html') : send(res, 404, 'viewer missing');
  }
...
// Probe a pretty hostname (e.g. exgram.localhost) over https THEN http, and
// return the base URL (scheme://host) whose /whoami reports OUR workspace — i.e.
// a local reverse proxy (portless.sh, …) is actually fronting this server. We try
// https first because portless serves TLS by default; we accept whatever it
// exposes and add no TLS config of our own. Returns null when the host doesn't
// resolve, doesn't answer, or fronts a different workspace -> caller falls back
// to the loopback port. Self-signed proxy certs are fine (rejectUnauthorized off).
const probePrettyBase = async (host) => {
  for (const [getter, scheme] of [[httpsGet, 'https'], [httpGet, 'http']]) {
    const who = await new Promise((resolve) => {
      let settled = false;
      const done = (v) => { if (!settled) { settled = true; resolve(v); } };
      const req = getter({ host, path: '/whoami', timeout: 500, rejectUnauthorized: false }, (res) => {
    ...
    if (who && who.workspace === absWorkspace) return `${scheme}://${host}`;
  }
  return null;
};
...
const start = async () => {
  ...
  for (let port = PREFERRED_PORT; port <= PREFERRED_PORT + MAX_PORT_PROBE; port++) {
    ...
    if (result === true) {
      ...
      // Prefer a pretty host (exgram.localhost) when a proxy actually fronts us;
      // the loopback port always works and is the fallback.
      const pretty = await probePrettyBase(PRETTY_HOST);
      const base = pretty || portBase;
      try {
      ...
      if (pretty) console.error(`exgram: (also reachable on ${portBase})`);
      return;
    }
