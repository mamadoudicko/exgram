...
import { createReadStream, existsSync, readdirSync, readFileSync, writeFileSync, unlinkSync, renameSync } from 'node:fs';
...
const handler = async (req, res) => {
  ...
  if ((req.method === 'PUT' || req.method === 'POST') && url.startsWith('/scene/')) {
    ...
    req.on('end', () => {
      if (aborted) return;
      try {
        writeFileSync(join(WORKSPACE, `${slug}.excalidraw`), Buffer.concat(chunks));
        send(res, 200, 'saved');
      } catch (err) {
